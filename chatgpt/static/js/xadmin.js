;!function (win) {
  ...
  Xadmin.prototype.updateFinanceData = function(){
    console.log("update begin...")
    $.ajax({
        url: "/finance/UpdateData/",
        type: "post",
        data: {"type":""},
        cache: false,
        dataType: "json",
        success: function(respond, status){
            if(status == "success" && respond["status"]=="OK"){
                var output = "update completed !"
                console.log(output)
                document.getElementById("update_console").innerHTML=output
            }else{
              var output = "update failed !"
                console.log(output)
                document.getElementById("update_console").innerHTML=output
